...
export default function RestCard({RestuInfo}){
    return(
        <Link to={"/city/Kolkata/"+RestuInfo?.info?.id}>
            ...
            <div className="max-w-[280px] mb-3 transform transition duration-200 hover:scale-95">
                <div>
                    <img className="w-70 h-45 object-cover rounded-2xl" src={"https://media-assets.swiggy.com/swiggy/image/upload/"+RestuInfo?.info?.cloudinaryImageId}></img>
                </div>

                <div className=" w-[92%] mx-auto ">
                    <div className="font-bold text-x"> {RestuInfo?.info?.name} </div>
